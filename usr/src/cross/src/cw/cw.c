...
//	newae(ctx->i_ae, "-fno-asm");
	...
	while (--ctx->i_oldargc > 0) {
		...
		case 'W':
			...
			if (strncmp(arg, "-Wl,-M", 6) == 0) {
				int len = strlen("-Wl,--version-script=") + strlen(arg + 6);
				char *s = malloc(len + 1);
				strcpy(s, "-Wl,--version-script=");
				strcat(s, arg + 6);
				newae(ctx->i_ae, s);
				free(s);
				break;
			}
			if (strncmp(arg, "-Wl,-zassert-deflib", strlen("-Wl,-zassert-deflib")) == 0 ||
			    strncmp(arg, "-Wl,-zguidance", strlen("-Wl,-zguidance")) == 0 ||
			    strncmp(arg, "-Wl,-zfatal-warnings", strlen("-Wl,-zfatal-warnings")) == 0
			   ) {
				break;
			}
	...
	if (!nolibc) {
		newae(ctx->i_ae, "-lgcc_s");
		newae(ctx->i_ae, "-lgcc");
		newae(ctx->i_ae, "-lc");
		newae(ctx->i_ae, "-lgcc_s");
		newae(ctx->i_ae, "-lgcc");
	}
	else if (op == CW_O_LINK)
		newae(ctx->i_ae, "-lgcc");
