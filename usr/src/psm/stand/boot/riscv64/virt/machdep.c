...
init_iolist(void)
{
	prom_walk(add_iomap, "ns16550a");
	prom_walk(add_iomap, "virtio,mmio");
	prom_walk(add_iomap, "riscv,plic0");
	prom_walk(add_iomap, "riscv,clint0");
...
void exitto(int (*entrypoint)())
{
	for (struct memlist *ml = plinearlistp; ml != NULL; ml = ml->ml_next) {
		...
		map_phys(PTE_A | PTE_D | PTE_G | PTE_W | PTE_R | pte_mt_mem, (caddr_t)(SEGKPM_BASE + pa), pa, sz);
	}
	for (struct memlist *ml = piolistp; ml != NULL; ml = ml->ml_next) {
		uintptr_t pa = ml->ml_address;
		uintptr_t sz = ml->ml_size;
		map_phys(PTE_A | PTE_D | PTE_G | PTE_W | PTE_R | pte_mt_io, (caddr_t)(SEGKPM_BASE + pa), pa, sz);
	}
	fini_memory();
...
set_rootfs(char *bpath, char *fstype)
{
	...
	if (strcmp(fstype, "nfs") == 0) {
		...
		if (strstr(str, "blkdev") == NULL) {
			static char buf[256];
			if (boot_partition >= 0)
				sprintf(buf, "%s/blkdev@0,0:%c", str, 'a' + boot_partition);
			else
