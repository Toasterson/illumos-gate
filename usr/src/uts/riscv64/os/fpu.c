...
extern void _fpu_save(fpu_ctx_t *);
extern void _fpu_restore(fpu_ctx_t *);
static void fpsave_ctxt(void *);
static void fprestore_ctxt(void *);
...
{
	csr_set_sstatus(SSR_FS_DIRTY);
	_fpu_save(fp);
	csr_clear_sstatus(SSR_FS_MASK);
}
...
fp_fenflt(void)
{
	...
	while (ctx != NULL) {
		if (ctx->save_op == (void(*)(void *))fp_save) {
			break;
		}
		ctx = ctx->next;
	}
...
void fp_init(void)
{
	...
	fp->fpu_regs.kfpu_csr = FCSR_INIT;
	fprestore_ctxt(fp);
