...
#if defined(DEBUG)
	ASSERT(lwp->lwp_ap == (long *)&lwptoregs(lwp)->r_ra);
#endif	/* DEBUG */
	...
	if (PTOU(p)->u_systrap) {
		if (prismember(&PTOU(p)->u_entrymask, code)) {
			...
			if (PTOU(p)->u_systrap &&
				...
				lwp->lwp_argsaved = 0;
				lwp->lwp_ap = (long *)&lwptoregs(lwp)->r_ra;
			}
			mutex_exit(&p->p_lock);
		}
		repost = 1;
...
save_syscall_args()
{
	...
	lwp->lwp_arg[0] = rp->r_a0;
	lwp->lwp_arg[1] = rp->r_a1;
	lwp->lwp_arg[2] = rp->r_a2;
	lwp->lwp_arg[3] = rp->r_a3;
	lwp->lwp_arg[4] = rp->r_a4;
	lwp->lwp_arg[5] = rp->r_a5;
	lwp->lwp_arg[6] = rp->r_a6;
	lwp->lwp_arg[7] = rp->r_a7;
...
/*
 * Indirect syscall handled in libc on RISC-V architectures
 */
int64_t
indir()
{
