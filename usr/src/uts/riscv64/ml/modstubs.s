...
#define STUB_COMMON(module, fcnname, install_fcn, retfcn, weak)		\
	...
	lw	t1, MODS_FLAG(t0);					\
	beqz	t1, 1f;				/* not weak */		\
	andi	t2, t1, MODS_INSTALLED;					\
	bnez	t2, 1f;				/* installed? yes */	\
	ld	t1, MODS_RETFCN(t0);					\
	jr	t1;			/* no, just jump to retfcn */	\
1:	j	stubs_common_code;					\
	SET_SIZE(fcnname);						\
...
#define STUB_UNLOADABLE(module, fcnname, install_fcn, retfcn, weak)	\
	...
	andi	t2, t1, MODS_INSTALLED;					\
	bnez	t2, 1f;				/* installed? yes */	\
	andi	t2, t1, MODS_WEAK;					\
	beqz	t2, 2f;				/* weak? no */		\
	ld	t1, MODS_RETFCN(t0);					\
	jr	t1;			/* yes, just jump to retfcn */	\
1:	ld	t1, MODS_INSTFCN(t0);/* just jump to install_fcn */	\
	jr	t1;							\
2:	j	stubs_common_code;					\
	SET_SIZE(fcnname);						\
	STUB_DATA(module, fcnname, install_fcn, retfcn, weak)
/*
 * We branch here with the fcnname_info pointer in t0.
 */
...
1:
	...
	.rept	((MAXNARG - 8) / 2)
	ld	t1, ((MAXNARG - 8) * 8)(sp)
	ld	t2, ((MAXNARG - 8 + 1) * 8)(sp)
	addi	sp, sp, -16
	sd	t1, 0(sp)
	sd	t2, 8(sp)
	.endr
	...
	/* Call stub function */
	ld	t0,  ((MAXNARG - 8) * 8)(sp)
	ld	t1, MODS_INSTFCN(t0)
	jalr	t1

	/* Preserve any return values */
	sd	a0, (sp)
	ld	a0, ((MAXNARG - 8) * 8)(sp)
	call	mod_release_stub

	/* Restore stack state and return. */
	ld	a0, (sp)
	ld	ra, ((MAXNARG - 8 + 1) * 8)(sp)
	addi	sp, sp, ((MAXNARG - 8 + 2) * 8)
	ret
 ...
 */
	...
	NO_UNLOAD_STUB(platmod, plat_hwclock_set_rate, nomod_minus_one);
	NO_UNLOAD_STUB(platmod, plat_hwclock_get_rate, nomod_minus_one);
