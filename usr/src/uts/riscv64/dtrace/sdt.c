...
#define	SDT_PATCHVAL16	0x9002		// c.ebreak
#define	SDT_PATCHVAL32	0x00100073	// ebreak
...
sdt_invop(uintptr_t addr, uintptr_t *stack, uintptr_t rval)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == addr) {
			...
			dtrace_probe(sdt->sdp_id, rp->r_a0, rp->r_a1, rp->r_a2, rp->r_a3, rp->r_a4);
...
sdt_provide_module(void *arg, struct modctl *ctl)
{
	...
	for (sdpd = mp->sdt_probes; sdpd != NULL; sdpd = sdpd->sdpd_next) {
		...
		uint32_t instr = *sdp->sdp_patchpoint;
		if ((instr & 0x3) != 0x3)
			instr &= 0xffff;
		sdp->sdp_patchval = (((instr & 0x3) == 0x3)? SDT_PATCHVAL32: SDT_PATCHVAL16);
		sdp->sdp_savedval = instr;
	}
}
...
static pfn_t
sdt_va_to_pfn(uintptr_t vaddr)
{
	uint64_t satp = csr_read_satp();
	int max_level = ((satp & SATP_MODE_MASK) == SATP_MODE_SV39)? 2:
		((satp & SATP_MODE_MASK) == SATP_MODE_SV48)? 3: -1;

	pte_t *pt = (pte_t *)hat_kpm_mapin_pfn(satp & SATP_PPN_MASK);
	for (int l = max_level; l >= 0; l--) {
		pte_t pte = pt[(vaddr >> (MMU_PAGESHIFT + l * __builtin_ctz(NPTEPERPT))) & (NPTEPERPT - 1)];
		if ((pte & PTE_V) == 0)
			return (PFN_INVALID);
		if (l == 0 || !IS_TABLE(pte))
			return PTE_TO_PFN(pte);
		pt = (pte_t *)hat_kpm_mapin_pfn(PTE_TO_PFN(pte));
	}
	return (PFN_INVALID);
}
...
{
	pfn_t pfn = sdt_va_to_pfn((uintptr_t)inst);
	if (pfn == PFN_INVALID)
		return -1;
	...
	if ((val & 3) == 3) {
		*(uint32_t *)((uintptr_t)vaddr + ((uintptr_t)inst & MMU_PAGEOFFSET)) = val;
		sync_icache((caddr_t)vaddr + ((uintptr_t)inst & MMU_PAGEOFFSET), sizeof(uint32_t));
	} else {
		*(uint16_t *)((uintptr_t)vaddr + ((uintptr_t)inst & MMU_PAGEOFFSET)) = val;
		sync_icache((caddr_t)vaddr + ((uintptr_t)inst & MMU_PAGEOFFSET), sizeof(uint16_t));
	}
...
sdt_enable(void *arg, dtrace_id_t id, void *parg)
{
	...
	while (sdp != NULL) {
		sdt_write_instruction(sdp->sdp_patchpoint, sdp->sdp_patchval);
		sdp = sdp->sdp_next;
	}
err:
	return (0);
...
sdt_disable(void *arg, dtrace_id_t id, void *parg)
{
	...
	while (sdp != NULL) {
		sdt_write_instruction(sdp->sdp_patchpoint, sdp->sdp_savedval);
		sdp = sdp->sdp_next;
	}
