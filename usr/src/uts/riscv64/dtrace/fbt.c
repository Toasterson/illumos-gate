...
#define FBT_PATCHVAL	0x00100073	// ebreak
#define FBT_NOP		0x00000013
#define FBT_RET		0x00008067
...
fbt_invop(uintptr_t addr, uintptr_t *stack, uintptr_t rval)
{
	...
	for (; fbt != NULL; fbt = fbt->fbtp_hashnext) {
		if ((uintptr_t)fbt->fbtp_patchpoint == addr) {
			CPU->cpu_dtrace_caller = rp->r_ra;
			if (fbt->fbtp_roffset == 0) {
				dtrace_probe(fbt->fbtp_id, rp->r_a0, rp->r_a1, rp->r_a2, rp->r_a3, rp->r_a4);
			} else {
...
fbt_provide_module(void *arg, struct modctl *ctl)
{
	...
	for (i = 1; i < nsyms; i++) {
		...
		uintptr_t st_value = sym->st_value;
		if (sym->st_shndx != SHN_ABS) {
			st_value += (uintptr_t)mp->dso;
		}
		instr = (uint32_t *)st_value;
...
again:
		...
		/*
		 * We only instrument "ret" on riscv64
		 */
		if (*instr != FBT_RET) {
			instr++;
			goto again;
...
{
	pfn_t pfn = fbt_va_to_pfn((uintptr_t)inst);
	if (pfn == PFN_INVALID)
		return -1;
