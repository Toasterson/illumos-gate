...
sync_instruction_memory(caddr_t addr, size_t len)
{
	...
	for (uintptr_t v = ((uintptr_t)addr & ~(data_line_size - 1)); v < (uintptr_t)addr + len; v += data_line_size) {
		clean_data_cache_pou(v);
	}
	dsb(ish);
	for (uintptr_t v = ((uintptr_t)addr & ~(inst_line_size - 1)); v < (uintptr_t)addr + len; v += inst_line_size) {
