...
sdt_reloc_resolve(struct module *mp, char *symname, uintptr_t jmpslot, uint_t rtype, int nreloc)
{
	...
	for (i = 0; i < nreloc; i++) {
		...
		if ((plt[0] & 0x9f00001f) != 0x90000010)
			return (1);

		// adrp x16, <label>
		uint64_t offset = (((plt[0] >> 5) & ((1u << 19) - 1)) << 2) | ((plt[0] >> 29) & ((1u << 2) - 1));
		offset = sign_extend(offset, 21);
		uintptr_t slot = ((uintptr_t)(&plt[0]) & ~0xffful) + (offset << 12);

		// ldr x17, [x16, #<pimm>]
		if ((plt[1] & 0xffc003ff) != 0xf9400211)
			return (1);
		offset = ((plt[1] >> 10) & ((1u << 12) - 1));
		if (slot + offset * 8 == jmpslot)
			break;
	}
	if (i == nreloc) {
		return 1;
	...
	for (size_t off = 0; off < text_size; off += 4) {
		...
		if ((instr & 0xfc000000) != 0x94000000)
			continue;
		if (off + extract_literal_offset(instr, 0, 26) == plt_offset) {
			...
			mp->sdt_probes = sdp;
			//_kobj_printf(ops, "sdt found %lx %s\n", off, mp->filename);
