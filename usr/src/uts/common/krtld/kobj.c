...
load_dso(struct modctl *modp, struct module *mp, struct _buf *file)
{
	...
	mp->dso_text = (uintptr_t)mp->dso + phdr_text.p_vaddr;
	mp->dso_text_size = phdr_text.p_filesz;

	if (phdr_data.p_memsz != 0) {
		mp->dso_data = (uintptr_t)mp->dso + phdr_data.p_vaddr;
		mp->dso_data_size = phdr_data.p_filesz;
		if (phdr_data.p_memsz - phdr_data.p_filesz) {
			mp->dso_bss = mp->dso_data + mp->dso_data_size;
			mp->dso_bss_size = phdr_data.p_memsz - phdr_data.p_filesz;
		}
	}
	...
	/* protect */
	as_setprot(&kas, mp->dso, mp->dso_size, PROT_READ);
	for (i = 0; i < ehdr.e_phnum; i++) {
		Phdr *phdr = (Phdr *)(phdrbase + ehdr.e_phentsize * i);
