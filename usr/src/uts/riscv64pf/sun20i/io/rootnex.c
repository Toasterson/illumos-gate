...
/*
 * XXX - does riscv64 even need these or are they left over from the SPARC days?
 */
	...
	case DDI_INTROP_ENABLE:
		{
			...
			int vec = ntohl((uint32_t)irupts_prop[interrupt_cells * hdlp->ih_inum + 0]);
			kmem_free(irupts_prop, irupts_len);
    ...
    off_t off, size_t len, uint_t cache_flags)
{
	...
	for (int i = 0; i < ncookies; i++) {
		...
		if (cur <= off && off < cur + map_size) {
			...
			thead_dcache_flush_phys(paddr + (off - cur), op_end);
			off = op_end;
		}
		cur += map_size;
		if (len != 0 && off == end)
	...
	while (size > 0) {
		...
		if (cbpage->pm_uses_copybuf) {
			 ...
			 */
				...
				thead_dcache_flush((uintptr_t)fromaddr, psize);
			}

			bcopy(fromaddr, toaddr, psize);

			if (cache_flags == DDI_DMA_SYNC_FORDEV) {
				thead_dcache_flush((uintptr_t)toaddr, psize);
			}
		}
