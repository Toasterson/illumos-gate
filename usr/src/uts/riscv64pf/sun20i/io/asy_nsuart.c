...
static int asymaxchip = ASY16750;	/* tunable: limit chip support we look for */
...
static void	async_flowcontrol_hw_output(struct asycom *asy,
		    async_flowc_action onoff);
static uint8_t	asy_get8(struct asycom *asy, int reg);
static void	asy_put8(struct asycom *asy, int reg, uint8_t val);
static void
asy_put8(struct asycom *asy, int reg, uint8_t val)
{
	switch (asy->asy_regshift) {
	case 0:
		ddi_put8(asy->asy_iohandle, (uint8_t *)(asy->asy_ioaddr + (reg << 0)), val);
		break;
	case 1:
		ddi_put16(asy->asy_iohandle, (uint16_t *)(asy->asy_ioaddr + (reg << 1)), val);
		break;
	case 2:
		ddi_put32(asy->asy_iohandle, (uint32_t *)(asy->asy_ioaddr + (reg << 2)), val);
		break;
	default:
		ASSERT(1);
		break;
...
static uint8_t
asy_get8(struct asycom *asy, int reg)
{
	uint8_t val;
	switch (asy->asy_regshift) {
	case 0:
		val = ddi_get8(asy->asy_iohandle, (uint8_t *)(asy->asy_ioaddr + (reg << 0)));
		break;
	case 1:
		val = ddi_get16(asy->asy_iohandle, (uint16_t *)(asy->asy_ioaddr + (reg << 1)));
		break;
	case 2:
		val = ddi_get32(asy->asy_iohandle, (uint32_t *)(asy->asy_ioaddr + (reg << 2)));
		break;
	default:
		ASSERT(1);
		break;
...
static uint_t
asy_divisor(struct asycom *asy, int bidx)
{
	if (bidx == 0)
		return 0;
	ASSERT(bidx < N_SU_SPEEDS);
	int baudrate = baudtable[bidx];
	return (asy->asy_clock + baudrate * 8) / (baudrate * 16);
}
...
asydetach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	switch (cmd) {
	case DDI_DETACH:
		DEBUGNOTE1(ASY_DEBUG_INIT, "asy%d: shutdown.",
		    instance);
...
asyattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	int regshift = 0;
	if (prom_getproplen(ddi_get_nodeid(devi), "reg-shift") == sizeof(int)) {
		prom_getprop(ddi_get_nodeid(devi), "reg-shift", (caddr_t)&regshift);
		regshift = ntohl(regshift);
	}

	if (prom_getproplen(ddi_get_nodeid(devi), "clock-frequency") == sizeof(uint_t)) {
		prom_getprop(ddi_get_nodeid(devi), "clock-frequency", (caddr_t)&uart_clock);
	...
	int fifo_size = 1;
	if (prom_getproplen(ddi_get_nodeid(devi), "fifo-size") == sizeof(int)) {
		prom_getprop(ddi_get_nodeid(devi), "fifo-size", (caddr_t)&fifo_size);
		fifo_size = ntohl(fifo_size);
	}
	asy->asy_fifo_buf = fifo_size;
	asy->asy_use_fifo = (fifo_size == 1? FIFO_ON: FIFO_OFF);
	asy->asy_fifor = FIFO_ON | FIFO_TRIG_1;
	...
	asy_put8(asy, LCR, DLAB);
	/* Set the baud rate to 9600 */
	asy_put8(asy, (DAT+DLL),
	    asy_divisor(asy, asy->asy_bidx) & 0xff);
	asy_put8(asy, (DAT+DLH),
	    (asy_divisor(asy, asy->asy_bidx) >> 8) & 0xff);
	asy_put8(asy, LCR, asy->asy_lcr);
	asy_put8(asy, MCR, mcr);
...
asyclose(queue_t *q, int flag, cred_t *credp)
{
	...
	if (async->async_flags & ASYNC_OUT_SUSPEND) {
		...
		mutex_enter(&asy->asy_excl_hi);
		lcr = asy_get8(asy, LCR);
		asy_put8(asy, LCR, (lcr & ~SETBREAK));
		mutex_exit(&asy->asy_excl_hi);
...
asy_program(struct asycom *asy, int mode)
{
	...
	(void) asy_get8(asy, ISR);
	(void) asy_get8(asy, LSR);
	asy->asy_msr = flush_reg = asy_get8(asy, MSR);
	/*
	...
	if (ocflags != (c_flag & ~CLOCAL) || mode == ASY_INIT) {
		...
		if (baudrate != 0) {
			asy_put8(asy, DAT,
			    asy_divisor(asy, baudrate) & 0xff);
			asy_put8(asy, ICR,
			    (asy_divisor(asy, baudrate) >> 8) & 0xff);
		}
		/* set the line control modes */
		asy_put8(asy, LCR, lcr);
...
asy_baudok(struct asycom *asy)
{
	...
	return (baudrate == 0 || asy_divisor(asy, baudrate));
}
...
asyintr(caddr_t argasy)
{
	...
	interrupt_id = asy_get8(asy, ISR) & 0x0F;
	async = asy->asy_priv;
	...
	if (async->async_ext) {
		...
		if (asy->asy_msr & DCD) {
			...
			if ((async->async_flags & ASYNC_CARR_ON) &&
				 ...
				 */
				val = asy_get8(asy, MCR);
				asy_put8(asy, MCR, (val & ~DTR));
	...
	if (error == 0) {
		...
		case TCSBRK:
			...
			if (*(int *)mp->b_cont->b_rptr == 0) {
				...
				while ((asy_get8(asy, LSR) & XSRE) == 0) {
					mutex_exit(&asy->asy_excl_hi);
					mutex_exit(&asy->asy_excl);
					drv_usecwait(
					    32*asy_divisor(asy, index) & 0xfff);
					mutex_enter(&asy->asy_excl);
					mutex_enter(&asy->asy_excl_hi);
				}
				/*
...
asymctl(struct asycom *asy, int bits, int how)
{
	...
	case TIOCMGET:
		 ...
		 */
		if (asy_get8(asy, ICR) & MIEN) {
			msr_r = asy->asy_msr;
			DEBUGCONT2(ASY_DEBUG_MODEM,
			    "asy%dmctl: TIOCMGET, read msr_r = %x\n",
			    instance, msr_r);
		} else {
			msr_r = asy_get8(asy, MSR);
			DEBUGCONT2(ASY_DEBUG_MODEM,
