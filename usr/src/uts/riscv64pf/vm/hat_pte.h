...
#define	PTE_NOSYNC		(0x1ul << PTE_SFW_SHIFT)
#define	PTE_NOCONSIST		(0x2ul << PTE_SFW_SHIFT)

#define	PTE_ISVALID(pte)	(((pte) & PTE_V) != 0)
#define	PTE_EQUIV(a, b)		(((a) | PTE_A | PTE_D) == ((b) | PTE_A | PTE_D))
#define	PTE_ISPAGE(p, l)	(PTE_ISVALID(p) && (((l) == PAGE_LEVEL) || (((p) & (PTE_X | PTE_W | PTE_R)) != 0)))

#define	MAKEPTE(pfn, l)		(PTE_FROM_PFN(pfn) | PTE_R | PTE_V)
#define	MAKEPTP(pfn, l)		(PTE_FROM_PFN(pfn) | PTE_V)
...
#define	PT_INDEX_PTR(p, x)	((volatile pte_t *)((uintptr_t)(p) + ((x) << PTE_BITS)))
