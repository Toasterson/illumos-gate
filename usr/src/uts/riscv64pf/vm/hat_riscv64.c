...
hat_alloc(struct as *as)
{
	...
	ht = htable_create(hat, (uintptr_t)0, mmu.max_level, NULL);	// create top level
	hat->hat_htable = ht;

	// for kernel space
	memcpy(
	    (void *)(SEGKPM_BASE + pfn_to_pa(ht->ht_pfn) + MMU_PAGESIZE / 2),
	    (void *)(SEGKPM_BASE + pfn_to_pa(kas.a_hat->hat_htable->ht_pfn) + MMU_PAGESIZE / 2),
	    MMU_PAGESIZE / 2);
...
get_cpu_node()
{
	...
	while (child > 0) {
		if (get_cpu_hartmask(child) == CPU->cpu_hartmask)
			return child;
...
static bool
is_supported_pbmt(pnode_t node)
{
...
static bool
is_supported_thead_mmu(pnode_t node)
{
	return prom_is_compatible(node, "thead,c906");
}
...
mmu_init(void)
{
	...
	hole_end = HOLE_END;
	csr_set_satp(SATP_ASID_MASK);
	mmu.max_asid = ((csr_read_clear_satp(SATP_ASID_MASK) & SATP_ASID_MASK) >> SATP_ASID_SHIFT);
	uint64_t satp = csr_read_satp();
	ASSERT(
	    ((satp & SATP_MODE_MASK) == SATP_MODE_SV39) ||
	    ((satp & SATP_MODE_MASK) == SATP_MODE_SV48));
	if ((satp & SATP_MODE_MASK) == SATP_MODE_SV39) {
		mmu.max_level = 2;
		mmu.kernelbase = ~((1ul << (39 - 1)) - 1);
	}
	if ((satp & SATP_MODE_MASK) == SATP_MODE_SV48) {
		mmu.max_level = 3;
		mmu.kernelbase = ~((1ul << (48 - 1)) - 1);
	}
	pnode_t node = get_cpu_node();
	if (node > 0 && is_supported_thead_mmu(node)) {
		mmu.mt_mem = PTE_MT_THEAD_MEM;
		mmu.mt_nc = PTE_MT_THEAD_NC;
		mmu.mt_io = PTE_MT_THEAD_IO;
	}
	else if (node > 0 && is_supported_pbmt(node)) {
		mmu.mt_mem = PTE_MT_PBMT_PMA;
		mmu.mt_nc = PTE_MT_PBMT_NC;
		mmu.mt_io = PTE_MT_PBMT_IO;
	} else {
		mmu.mt_mem = 0;
		mmu.mt_nc = 0;
		mmu.mt_io = 0;
...
hat_switch(hat_t *hat)
{
	...
	if (old != hat) {
		...
		csr_write_satp(SATP_MODE_SV39 | ((uint64_t)hat->hat_asid[cpuid] << SATP_ASID_SHIFT) | hat->hat_htable->ht_pfn);
		if (req_tlbi) {
...
hati_mkpte(pfn_t pfn, uint_t attr, level_t level, uint_t flags)
{
	...
	if (flags & HAT_LOAD_NOCONSIST)
		PTE_SET(pte, PTE_NOCONSIST | PTE_A | ((attr & PROT_WRITE)? PTE_D: 0));
	else if (attr & HAT_NOSYNC)
		PTE_SET(pte, PTE_NOSYNC | PTE_A | ((attr & PROT_WRITE)? PTE_D: 0));

	switch (cache_attr) {
	case HAT_STRICTORDER:
		PTE_SET(pte, mmu.mt_io);
		break;
	case HAT_MERGING_OK:
		PTE_SET(pte, mmu.mt_nc);
		break;
	case HAT_STORECACHING_OK:
		PTE_SET(pte, mmu.mt_mem);
		break;
	default:
...
hati_sync_pte_to_page(page_t *pp, pte_t pte, level_t level)
{
	...
	switch (PTE_GET(pte, PTE_A | PTE_D)) {
	case PTE_A | PTE_D:
		rm |= (P_REF | P_MOD);
		break;
	case PTE_A:
...
#define	PTE_REMAP_BITS							\
	(PTE_PFN_MASK | PTE_D | PTE_A | PTE_X | PTE_W)
	...
	pfn_t		pfn)
{
	...
	if ((attr & PROT_EXEC) || (attr & HAT_ORDER_MASK) != HAT_STORECACHING_OK)
		sync_data_memory(hat_kpm_pfn2va(pfn), LEVEL_SIZE(level));
...
hat_unlock_region(struct hat *hat, caddr_t addr, size_t len,
    hat_region_cookie_t rcookie)
{
	panic("No shared region support on risc-v");
}
...
hat_tlb_inval_range(hat_t *hat, uintptr_t va, size_t len)
{
	...
	if (va == DEMAP_ALL_ADDR) {
		struct sbiret ret = sbi_remote_sfence_vma(0, -1ul, 0, 0);
		ASSERT(ret.error == 0);
	} else {
		struct sbiret ret = sbi_remote_sfence_vma(0, -1ul, va, len);
		ASSERT(ret.error == 0);
	}
	kpreempt_enable();
}
...
hat_kmap_unload(caddr_t addr, size_t len, uint_t flags)
{
	...
	for (; va < eva; va += MMU_PAGESIZE) {
		 ...
		 */
		hat_pte_unmap(ht, entry, flags, old_pte, (void *)pte_ptr, B_TRUE);
	}
}
	...
	hat_callback_t	*cb)
{
	...
	while (vaddr < eaddr) {
		...
		ASSERT(ht->ht_level <= mmu.max_level);
		vaddr += LEVEL_SIZE(ht->ht_level);
		contig_va = vaddr;
		++r[r_cnt - 1].rng_cnt;
	}
...
hat_flush_range(hat_t *hat, caddr_t va, size_t size)
{
	...
	while (va < endva) {
		...
		if (sz < 0) {
			...
			struct sbiret ret = sbi_remote_sfence_vma(0, -1ul, (uintptr_t)va, sz);
			ASSERT(ret.error == 0);
		}
		kpreempt_enable();
...
hat_getattr(hat_t *hat, caddr_t addr, uint_t *attr)
{
	...
	ht = htable_getpte(hat, vaddr, NULL, &pte, mmu.max_level);
	if (ht == NULL)
		return ((uint_t)-1);
	...
	*attr = PROT_READ;
	if (PTE_GET(pte, PTE_W))
		*attr |= PROT_WRITE;
	if (PTE_GET(pte, PTE_U)) {
		*attr |= PROT_USER;
		if (PTE_GET(pte, PTE_X))
			*attr |= PROT_EXEC;
	} else {
		if (PTE_GET(pte, PTE_X))
			*attr |= PROT_EXEC;
...
try_again:
		...
		if (PTE_GET(newpte, PTE_SOFTWARE) >= PTE_NOSYNC)
			newpte |= PTE_A | (PTE_GET(newpte, PTE_W)? PTE_D: 0);
...
not_shared:
		...
		while (pgcnt > 0) {
			...
			prot = PROT_USER | PROT_READ | HAT_STORECACHING_OK;
			if (PTE_GET(pte, PTE_W))
				prot |= PROT_WRITE;
			if (PTE_GET(pte, PTE_X))
				prot |= PROT_EXEC;
...
clear_boot_mappings(uintptr_t low, uintptr_t high)
{
	...
	while (va < high) {
		...
		if (l > 0 && IS_TABLE(*ptbl[l])) {
			...
			if ((low <= va) && (va + LEVEL_SIZE(l) - 1 <= high - 1)) {
...
hati_update_pte(htable_t *ht, uint_t entry, pte_t expected, pte_t new)
{
	...
	if (
	    PTE_GET(expected, PTE_A | PTE_D) &&
	    (
	     (PTE_GET(expected, PTE_SOFTWARE) < PTE_NOSYNC && PTE_GET(new, PTE_NOSYNC)) ||
	     (PTE_GET(expected, PTE_D) && !PTE_GET(new, PTE_W)) ||
	     (PTE_GET(expected, PTE_A | PTE_D) != PTE_GET(new, PTE_A | PTE_D))
	     )
	    ) {
		...
		switch (PTE_GET(expected, PTE_A | PTE_D)) {
		case PTE_A:
			rm |= P_REF;
			break;
		case PTE_A | PTE_D:
...
int
hat_page_falt(hat_t *hat, caddr_t vaddr, enum seg_rw rw)
{
	int rv = -1;
	...
	for (;;) {
		uint_t entry;
		pte_t oldpte;
		htable_t *ht = htable_getpte(hat, (uintptr_t)vaddr, &entry, &oldpte, mmu.max_level);
		if (ht == NULL)
			break;

		if (!PTE_ISVALID(oldpte) || !PTE_ISPAGE(oldpte, ht->ht_level)) {
			htable_release(ht);
			break;
		}

		if (!PTE_GET(oldpte, PTE_R) ||
		    (rw == S_WRITE && !PTE_GET(oldpte, PTE_W)) ||
		    (rw == S_EXEC && !PTE_GET(oldpte, PTE_X))
		    ) {
			htable_release(ht);
			break;
		}

		if (!PTE_GET(oldpte, PTE_A) || (rw == S_WRITE && !PTE_GET(oldpte, PTE_D))) {
			...
			PTE_SET(newpte, PTE_A | (rw == S_WRITE && PTE_GET(oldpte, PTE_W)? PTE_D: 0));
