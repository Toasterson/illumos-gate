...
send_dirint(int cpuid, int irq)
{
	...
	sbi_send_ipi(cpu[cpuid]->cpu_hartmask, 0);
...
static uint64_t plic_base;

uint32_t plic_get_priority(int irq)
{
	ASSERT(0 < irq && irq < 512);
	return *(volatile uint32_t *)(SEGKPM_BASE + plic_base + irq * 4);
}
void plic_set_priority(int irq, uint32_t lvl)
{
	ASSERT(0 < irq && irq < 512);
	ASSERT(lvl <= plic_max_lvl);
	*(volatile uint32_t *)(SEGKPM_BASE + plic_base + irq * 4) = lvl;
}
bool plic_is_pending(int irq)
{
	ASSERT(0 < irq && irq < 512);
	return (*(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x1000 + (irq / 32) * 4) & (1u << (irq % 32))) != 0;
}
...
void plic_enable(int irq, int cpuid)
{
	...
	*(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x2000 + 0x80 * plic_id + (irq / 32) * 4) |= (1u << (irq % 32));
...
uint32_t plic_get_level(void)
{
	...
	return *(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x200000 + 0x1000 * plic_id);
}
void plic_set_level(uint32_t lvl)
{
	int plic_id = plic_id_table[CPU->cpu_id];
	*(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x200000 + 0x1000 * plic_id) = lvl;
}
int plic_get_irq(void)
{
	int plic_id = plic_id_table[CPU->cpu_id];
	return *(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x200004 + 0x1000 * plic_id);
}
void plic_eol(int irq)
{
	int plic_id = plic_id_table[CPU->cpu_id];
	*(volatile uint32_t *)(SEGKPM_BASE + plic_base + 0x200004 + 0x1000 * plic_id) = irq;
}
...
get_plic_node()
{
	...
	if (ic < 0) {
		ic = find_compatible(prom_rootnode(), "thead,c900-plic");
	}
	return ic;
}
...
plic_map(void)
{
	...
	if (len >= 0) {
		...
		for (int i = 0; i < len / (sizeof(uint32_t) * 2); i++) {
			...
			int arg = (int)htonl(extended[2 * i + 1]);
			if (arg != 9) // S-mode External
				continue;
			ASSERT(parent >= 0);
...
void intr_init(void)
{
	...
	prom_get_reg_address(ic, 0, &plic_base);
	ASSERT(plic_base != 0);
