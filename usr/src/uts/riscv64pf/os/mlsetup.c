 ...
 */
void
mlsetup(struct regs *rp, int hartid, uint64_t hartmask)
{
	extern char t0stack[];
	extern struct classfuncs sys_classfuncs;
	extern disp_t cpu0_disp;

	enable_hartmask = hartmask;

	/*
	 * initialize cpu_self
	 */
	asm volatile ("mv tp, %0"::"r"((uintptr_t)&t0):"tp");
	...
	CPU->cpu_id = 0;
	CPU->cpu_m.mcpu_hartmask = 1ul << hartid;
