...
tenmicrosec(void)
{
	...
	if (gethrtime_hires) {
		...
		uint64_t end_count = csr_read_time() + timer_freq / (MICROSEC / 10);

		while (csr_read_time() < end_count)
			SMT_PAUSE();
	}
}
...
lwp_stk_init(klwp_t *lwp, caddr_t stk)
{
	...
	oldstk = stk;
	stk -= SA(sizeof (greg_t) * 2);	// for tp and gp
...
panic_stopcpus(cpu_t *cp, kthread_t *t, int spl)
{
	...
	send_dirint(cp->cpu_id, IRQ_IPI_STOP);
}
