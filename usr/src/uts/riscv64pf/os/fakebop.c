...
pt_alloc(bootops_t *bop)
{
	...
	bzero(pa_to_kseg(pa), MMU_PAGESIZE);
	return pa;
}

static pte_t pte_mt_mem = 0;
static void
map_phys(bootops_t *bop, pte_t pte_attr, caddr_t vaddr, uint64_t paddr)
{
	...
	pte_t l2pte = l2pt[l2_pteidx((uintptr_t)vaddr)];
	if ((l2pte & PTE_V) == 0) {
		paddr_t pa = pt_alloc(bop);
		asm volatile ("fence":::"memory");
		l2pte = l2pt[l2_pteidx((uintptr_t)vaddr)] = PTE_FROM_PA(pa) | PTE_V;
	}
	ASSERT(IS_TABLE(l2pte));
	pte_t *l1pt = (pte_t *)pa_to_kseg(PTE_TO_PA(l2pte));
	pte_t l1pte = l1pt[l1_pteidx((uintptr_t)vaddr)];
	if ((l1pte & PTE_V) == 0) {
		paddr_t pa = pt_alloc(bop);
		asm volatile ("fence":::"memory");
		l1pte = l1pt[l1_pteidx((uintptr_t)vaddr)] = PTE_FROM_PA(pa) | PTE_V;
	}
	ASSERT(IS_TABLE(l1pte));
	pte_t *l0pt = (pte_t *)pa_to_kseg(PTE_TO_PA(l1pte));
	pte_t l0pte = l0pt[l0_pteidx((uintptr_t)vaddr)];
	ASSERT((l0pte & PTE_V) == 0);
	l0pt[l0_pteidx((uintptr_t)vaddr)] = pte_attr | PTE_FROM_PA(paddr) | PTE_V;
	asm volatile ("fence":::"memory");
}
...
do_bsys_alloc(bootops_t *bop, caddr_t virthint, size_t size, int align)
{
	...
	s = size;
	while (s > 0) {
		map_phys(bop, PTE_NOCONSIST | PTE_D | PTE_A | PTE_G | PTE_W | PTE_R | pte_mt_mem, va, pa);
		va += MMU_PAGESIZE;
		pa += MMU_PAGESIZE;
		s -= MMU_PAGESIZE;
	}
