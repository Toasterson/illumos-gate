...
static pnode_t
get_cpu_node_by_hartmask(uint64_t hartmask)
{
	...
	while (child > 0) {
		if (get_cpu_hartmask(child) == hartmask)
			return child;
...
init_cpu_info(struct cpu *cp)
{
	...
	pnode_t node = get_cpu_node_by_hartmask(cp->cpu_hartmask);
	if (node > 0 && prom_getproplen(node, "riscv,isa") > 0) {
		cp->cpu_isa = kmem_zalloc(prom_getproplen(node, "riscv,isa") + 1, KM_SLEEP);
		prom_getprop(node, "riscv,isa", cp->cpu_isa);
	} else {
		cp->cpu_isa = kmem_zalloc(32, KM_SLEEP);
		strcat(cp->cpu_isa, "rv64imafdc");
	}
	for (const char *c = cp->cpu_isa; *c != 0 && *c != '_'; c++) {
		if (*c == 'f')
		...
		if (*c == 'g')
			strcat(pi->pi_fputypes, "fd");
	}
	if (node > 0 && prom_getproplen(node, "mmu-type") > 0) {
		cp->cpu_mmu = kmem_zalloc(prom_getproplen(node, "mmu-type") + 1, KM_SLEEP);
		prom_getprop(node, "mmu-type", cp->cpu_mmu);
	} else {
		cp->cpu_mmu = kmem_zalloc(32, KM_SLEEP);
		strcat(cp->cpu_mmu, "riscv,sv39");
	}
	if (node > 0 && prom_getproplen(node, "compatible") > 0) {
		cp->cpu_uarch = kmem_zalloc(prom_getproplen(node, "compatible") + 1, KM_SLEEP);
		prom_getprop(node, "compatible", cp->cpu_uarch);
	} else {
		cp->cpu_uarch = kmem_zalloc(32, KM_SLEEP);
		strcat(cp->cpu_uarch, "riscv");
	}
...
static pnode_t
get_cpu_node(int cpun, uint64_t ignore_hartmask)
{
...
wakeup_cpu(int who, uint64_t ignore_hartmask)
{
	...
	int hartid = __builtin_ctzl(hartmask);
	uint64_t pa;
	if (!vtop((uintptr_t)secondary_vec_start, &pa))
		return -1;
	sbi_remote_fence_i(0, -1ul);
	struct sbiret ret = sbi_hart_start(hartid, pa, csr_read_satp());
	return ret.error;
}
