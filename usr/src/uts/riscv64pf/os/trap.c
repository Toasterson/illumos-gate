...
extern void dtrace_user_probe(struct regs *);
...
trap(uint64_t scause, uint64_t stval, struct regs *rp)
{
	...
	if (DTRACE_CPUFLAG_ISSET(CPU_DTRACE_NOFAULT)) {
		ASSERT((rp->r_ssr & SSR_SPIE) == 0);
		ASSERT(!USERMODE(rp->r_ssr));
		switch (scause) {
		case T_LACCESS:
		case T_LPGFLT:
		case T_SACCESS:
		case T_SPGFLT:
			...
			cpu_core[CPU->cpu_id].cpuc_dtrace_illval = stval;
			break;
		default:
			DTRACE_CPUFLAG_SET(CPU_DTRACE_ILLOP);
			break;
		}
		int inst_len = ((*((uint16_t *)rp->r_pc) & 0x3) == 0x3)? 4: 2;
		rp->r_pc += inst_len;
		return 0;
	}
	if (USERMODE(rp->r_ssr)) {
		if (scause == T_ILEXC) {
			...
			csr_set_sstatus(SSR_SUM);
			illop = *((uint16_t *)rp->r_pc);
			csr_clear_sstatus(SSR_SUM);
			DTRACE_CPUFLAG_CLEAR(CPU_DTRACE_NOFAULT);
			if (!DTRACE_CPUFLAG_ISSET(CPU_DTRACE_FAULT)) {
				if (illop == FASTTRAP_INSTR) {
					csr_set_sstatus(SSR_SIE);
					dtrace_user_probe(rp);
					return 0;
				}
			} else {
		...
		if (scause == T_BRK) {
			int r = dtrace_invop(rp->r_pc, (uintptr_t *)rp, rp->r_a0);
	...
	if (mstate != LMS_TRAP) {
		if ((caddr_t)_userlimit <= addr && addr < (caddr_t)kernelbase) {
			...
			if (!(!USERMODE(rp->r_ssr) && addr < (caddr_t)_userlimit && (rp->r_ssr & SSR_SUM) == 0)) {
				return 1;
			}
		} else {
	...
	if (USERMODE(rp->r_ssr)) {
		...
		int stepped = (lwp->lwp_pcb.pcb_step != STEP_NONE && prundostep() && scause == T_BRK);
		...
		case T_SPGFLT:
			{
				...
				if (res == 0 && !(addr < (caddr_t)_userlimit && (rp->r_ssr & SSR_SUM) == 0))
					goto cleanup;
