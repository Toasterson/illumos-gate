 ...
 */
uint_t
cbe_fire_master(caddr_t, caddr_t)
{
	cpu_t *cpu = CPU;
...
uint_t
cbe_fire_slave(caddr_t, caddr_t)
{
	cpu_t *cpu = CPU;
	processorid_t me = cpu->cpu_id, i;
...
cbe_init(void)
{
	...
	if (cbe_vector > 0) {
		(void) add_avintr(NULL, CBE_HIGH_PIL, cbe_fire_master,
		    "cbe_fire_master", cbe_vector, 0, NULL, NULL, NULL);
	}

	(void) add_avintr(NULL, CBE_HIGH_PIL, cbe_fire_slave,
