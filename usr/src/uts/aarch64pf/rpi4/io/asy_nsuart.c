...
static void
pl011_reset_fifo(struct asycom *asy, uint8_t flush)
{
}

static void
pl011_put_char(struct asycom *asy, uint8_t val)
{
	union uart_dr reg = {0};
	reg.data = val;
...
static bool
pl011_is_busy(struct asycom *asy)
{
	union uart_fr reg;
...
static bool
pl011_tx_is_ready(struct asycom *asy)
{
	union uart_fr reg;
	reg.dw = REG_READ(asy, UARTFR);
	return reg.txff == 0;
...
static bool
pl011_rx_is_ready(struct asycom *asy)
{
	union uart_fr reg;
	reg.dw = REG_READ(asy, UARTFR);
	return !reg.rxfe;
...
static uint8_t
pl011_get_msr(struct asycom *asy)
{
	union uart_fr reg;
	reg.dw = REG_READ(asy, UARTFR);
	uint8_t ret = 0;
...
static void
pl011_set_mcr(struct asycom *asy, uint8_t mcr)
{
	union uart_cr reg;
...
static uint8_t
pl011_get_mcr(struct asycom *asy)
{
	union uart_cr reg;
	reg.dw = REG_READ(asy, UARTCR);
	uint8_t ret = 0;
...
static void
pl011_set_icr(struct asycom *asy, uint8_t icr, uint8_t mask)
{
	union uart_intr reg;
...
static uint8_t
pl011_get_icr(struct asycom *asy)
{
	union uart_intr reg;
	reg.dw = REG_READ(asy, UARTIMSC);
	uint8_t ret = 0;
...
static uint8_t
pl011_get_lsr(struct asycom *asy)
{
	union uart_rsr rsr;
...
static uint8_t
pl011_get_char(struct asycom *asy)
{
	union uart_dr reg;
...
static void
pl011_set_lcr(struct asycom *asy, uint8_t lcr)
{
...
static void
pl011_set_break(struct asycom *asy, boolean_t on)
{
	union uart_lcr_h reg;
	reg.dw = REG_READ(asy, UARTLCR_H);
	reg.brk = (on? 1: 0);
...
static uint8_t
pl011_get_lcr(struct asycom *asy)
{
	union uart_lcr_h reg;
	reg.dw = REG_READ(asy, UARTLCR_H);
...
static uint8_t
pl011_get_isr(struct asycom *asy)
{
	union uart_intr reg;
...
static void
pl011_set_baud(struct asycom *asy, uint8_t bidx)
{
...
asyattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	pl011_set_baud(asy, asy->asy_bidx);
	pl011_set_lcr(asy, asy->asy_lcr);
	pl011_set_mcr(asy, mcr);
...
nodrain:
	...
	if ((async->async_ttycommon.t_cflag & HUPCL) ||
		...
		if (asy->asy_flags & (ASY_IGNORE_CD|ASY_RTS_DTR_OFF)) {
			...
			pl011_set_mcr(asy, asy->asy_mcr|OUT2);
		} else {
			DEBUGCONT1(ASY_DEBUG_MODEM,
 ...
 */
	return ((async->async_ocnt > 0) || pl011_is_busy(asy));
}
...
asy_program(struct asycom *asy, int mode)
{
	...
	if (mode == ASY_INIT)
		while (pl011_rx_is_ready(asy))
			pl011_get_char(asy);

	if (ocflags != (c_flag & ~CLOCAL) || mode == ASY_INIT) {
		/* Set line control */
		lcr = pl011_get_lcr(asy);
		...
		if (baudrate != 0)
			pl011_set_baud(asy, baudrate);

		/* set the line control modes */
		pl011_set_lcr(asy, lcr);
		...
		if ((ocflags & CREAD) == 0 && (c_flag & CREAD) ||
		    mode == ASY_INIT)
			pl011_reset_fifo(asy, FIFORXFLSH);
...
asyintr(caddr_t argasy)
{
	...
	interrupt_id = pl011_get_isr(asy);
	async = asy->asy_priv;
	if ((async == NULL) ||
		...
		if (interrupt_id & NOINTERRUPT)
			 ...
			 */
			(void) pl011_get_lsr(asy);
			(void) pl011_get_char(asy);
			asy->asy_msr = pl011_get_msr(asy);
...
async_txint(struct asycom *asy)
{
	...
	if (!(async->async_flags &
		...
		while (async->async_ocnt > 0) {
			if (!pl011_tx_is_ready(asy)) {
				break;
			}
			pl011_put_char(asy, *async->async_optr++);
			async->async_ocnt--;
		}
		async->async_flags |= ASYNC_PROGRESS;
	}
	...
	if (async->async_ext) {
		...
		if (asy->asy_msr & DCD) {
			...
			if ((async->async_flags & ASYNC_CARR_ON) &&
				 ...
				 */
				val = pl011_get_mcr(asy);
				pl011_set_mcr(asy, (val & ~DTR));
				...
				if (async->async_flags & ASYNC_ISOPEN) {
					 ...
					 */
					pl011_reset_fifo(asy, FIFOTXFLSH);
					/*
...
asyputchar(cons_polledio_arg_t arg, uchar_t c)
{
	...
	while (!pl011_tx_is_ready(asy)) {
		/* wait for xmit to finish */
		drv_usecwait(10);
	}

	/* put the character out */
	pl011_put_char(asy, c);
}
...
asymctl(struct asycom *asy, int bits, int how)
{
	...
	case TIOCMGET:
		 ...
		 */
		if (pl011_get_icr(asy) & MIEN) {
			msr_r = asy->asy_msr;
			DEBUGCONT2(ASY_DEBUG_MODEM,
			    "asy%dmctl: TIOCMGET, read msr_r = %x\n",
			    instance, msr_r);
		} else {
			msr_r = pl011_get_msr(asy);
			DEBUGCONT2(ASY_DEBUG_MODEM,
    ...
    int type)
{
	...
	if ((async->async_flags & (ASYNC_SW_IN_NEEDED | ASYNC_BREAK | ASYNC_OUT_SUSPEND)) == ASYNC_SW_IN_NEEDED) {
		...
		while (!pl011_tx_is_ready(asy)) {}
		pl011_put_char(asy, async->async_flags & ASYNC_SW_IN_FLOW ?  async->async_stopc : async->async_startc);
		rval = B_TRUE;
	}
	return (rval);
}
...
asyquiesce(dev_info_t *devi)
{
	...
	pl011_set_icr(asy, 0, RIEN | TIEN);

	/* reset the FIFO */
	pl011_reset_fifo(asy, FIFOTXFLSH | FIFORXFLSH);
